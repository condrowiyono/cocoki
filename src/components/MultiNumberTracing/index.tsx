...
import styles from "./MultiNumberTracing.module.scss";
import { NUMBER_DATA } from "../NumberTracing/constants";
import type { Line } from "../NumberTracing/type";

interface NumberTracerProps {
  number: number;
}

function NumberTracer({ number }: NumberTracerProps) {
  ...
  const checkPointValidity = (x: number, y: number) => {
    for (const path of numberData.paths) {
  ...
  return (
    <div className={styles.tracerCard}>
      <svg
        ref={canvasRef}
        viewBox="0 0 500 500"
...
interface MultiNumberTracingProps {
  number: number;
  count?: number;
}

function MultiNumberTracing({ number, count = 6 }: MultiNumberTracingProps) {
  const numberData = NUMBER_DATA[number];
  ...
  return (
    <div className={styles.multiNumberTracing}>
      {/* Header */}
      ...
      <div className={styles.mainContent}>
        ...
        {/* Grid of Tracers */}
        <div className={styles.tracersGrid}>
          {Array.from({ length: count }).map((_, index) => (
            <NumberTracer key={index} number={number} />
          ))}
        </div>
      </div>
    </div>
...
export default MultiNumberTracing;
